...
export default function PhotoboothPage({ params }) {
  ...
  const [step, setStep] = useState('CAPTURE'); // CAPTURE | REVIEW | UPLOADING | SUCCESS
    ...
    try {
      ...
      if (data.success && data.events) {
        ...
        if (found) {
          ...
          console.error(`Event dengan slug "${slug}" tidak ditemukan.`);
        }
      }
    } catch (err) {
  ...
  // 2. Inisialisasi Kamera Web (Hanya jika Event Aktif)
  useEffect(() => {
    const isEventDisabled =
      eventData && (eventData.is_active === false || eventData.status === 'offline');

    // Matikan stream kamera jika event ternyata nonaktif
    if (isEventDisabled && videoRef.current && videoRef.current.srcObject) {
    ...
    if (step === 'CAPTURE' && !isEventDisabled) {
  ...
  // Handler Hitung Mundur & Audio Trigger
  ...
  const renderPhotoStripBlob = () => {
    return new Promise((resolve) => {
      ...
      // --- 1. HEADER: JUDUL EVENT & TANGGAL/JAM ---
      ctx.fillStyle = '#0F172A';
      ctx.font = 'bold 38px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(slug ? slug.replace(/-/g, ' ').toUpperCase() : '', width / 2, 110);
      ...
      photos.forEach((src, idx) => {
        ...
        img.onload = async () => {
          ...
          if (loadedCount === 3) {
            ...
            const photoHeight = 390; // Rasio 4:3
            const startX = 40;
            const startY = 200;
            const gap = 25;
            ...
            ctx.fillStyle = '#64748B';
            ctx.fillText(textBase, width / 2 - 35, height - 70);

            ctx.fillStyle = '#4F46E5';
            ctx.font = 'bold 16px sans-serif';
            ctx.fillText(textLink, width / 2 + 130, height - 70);

            // --- 4. RENDER CUSTOM FRAME OVERLAY ---
            if (eventData?.frame_url) {
              const frameImg = new Image();
              frameImg.crossOrigin = 'anonymous';
              frameImg.src = eventData.frame_url;
              ...
              await new Promise((resolveFrame) => {
                ...
                frameImg.onerror = () => {
                  console.warn('Gagal memuat image frame overlay');
                  resolveFrame();
                };
              });
            }
  ...
  const handleFinalUpload = async () => {
    ...
    try {
      ...
      const file = new File([blob], `strip_${slug}_${Date.now()}.jpg`, { type: 'image/jpeg' });

      const formData = new FormData();
      formData.append('file', file);
      formData.append('tenantId', eventData?.tenant_id || tenantId);
  ...
  return (
    <main className="h-[100dvh] w-full bg-slate-100 text-slate-800 flex flex-col justify-between p-3 overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col items-center justify-center min-h-0 w-full max-w-xs mx-auto my-auto">
        {/* JIKA EVENT NONAKTIF / OFFLINE */}
        {isEventDisabled ? (
          <div className="w-full bg-white p-6 rounded-2xl shadow-xl border border-slate-200 text-center flex flex-col items-center justify-center my-auto space-y-3">
            ...
            <h2 className="text-base font-bold text-slate-800">Event Sedang Nonaktif</h2>
            <p className="text-xs text-slate-500 leading-relaxed">
              Sesi photobooth untuk acara ini telah ditutup atau sedang di-nonaktifkan oleh penyelenggara.
            </p>
          </div>
        ) : (
          <>
            {step === 'CAPTURE' && (
              <div className="w-full flex flex-col items-center justify-center space-y-3">
                ...
                {/* THUMBNAILS DENGAN TOMBOL INDIVIDUAL */}
            ...
            {step === 'REVIEW' && (
              <div className="w-full flex flex-col items-center justify-center my-auto space-y-3">
                {/* PHOTO STRIP - CENTERED */}
                <div className="w-[170px] aspect-[1/3] bg-white p-2.5 rounded-2xl shadow-xl border border-slate-200 flex flex-col justify-between relative overflow-hidden">
                  ...
                  <p className="text-center font-bold text-[9px] text-slate-800 tracking-wider uppercase mt-1.5">
                    {slug}
                  </p>

                  {/* Preview overlay jika frame terpasang */}
                  {eventData?.frame_url && (
                    <img
                      src={eventData.frame_url}
